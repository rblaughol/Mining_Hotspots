...
if __name__ == '__main__':
    ...
    for truck_dist in all_data_list:
        ...
        for i in range(len(space_list) - 1):
            # 根据step进行取样
            trajectory_tuple = tuple(space_list[i:i + step])
    ...
    for i in range(len(result)):
        location_1 = memory_list[int(list(result[i].keys())[0][0]) - 1]
        lon_1 = float(location_1[1])
        lat_1 = float(location_1[0])
        loc_1 = [lat_1, lon_1]

        location_2 = memory_list[int(list(result[i].keys())[0][1]) - 1]
        lon_2 = float(location_2[1])
        lat_2 = float(location_2[0])
        loc_2 = [lat_2, lon_2]
        ...
        result_line_str += str((loc_1,loc_2))
